...
The IBTrACS data are preprocessed by preprocess_tc_ibtracs.py.
"""
__author__ = "Zhe.Feng@pnnl.gov"
__date__ = "05-Mar-2024"
...
def get_mcs_tc_tracknumber(mcsfile, dstc):
    ...
    # if len(mcs_tracknumber_tc) > 0:
    #     import matplotlib.pyplot as plt
    #     import pdb; pdb.set_trace()

    print(f'Done processing {mcsfile}')
...
def write_netcdf(outfile, mcs_tracknumber, mcs_nhours):
    ...
    var_dict = {
        'mcs_tracknumber': (['tracks'], mcs_tracknumber), \
        'mcs_nhours': (['tracks'], mcs_nhours),            
    }
    coord_dict = {'tracks': (['tracks'], np.arange(0, ntracks, 1))}
    gattr_dict = {
        'title': 'MCS tracknumbers near Tropical Cyclones', \
...
if __name__ == '__main__':
    ...
    pixelfiles = sorted(glob.glob(f'{pixel_dir}{inyear}/mcstracking/{indates}/mcstrack_{inyear}????_????.nc'))
